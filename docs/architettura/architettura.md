...
*Documento di riferimento unico per le decisioni architetturali. Aggiornato al 2 settembre 2026.*

## Assunzioni di partenza

- Team: 2-4 persone, competenze miste.
- Hosting: cloud, budget contenuto (pochi €/mese). Il team gestisce già infrastruttura su DigitalOcean per un altro progetto interno e preferisce un solo provider invece di spezzare lo stack su tre servizi diversi.
...
Scelta: Postgres self-hosted, non un servizio managed a canone gratuito — un log eventi che deve crescere in continuo per mesi eccede in fretta i limiti di storage di un free tier, e comunque un free tier copre tipicamente solo lo storage, non il compute sempre acceso richiesto dal bot. Dato che il team gestisce già DigitalOcean, ha più senso avere Postgres in un container Docker sulla stessa droplet del bot/API — un solo provider, un solo bill, meno pezzi mobili da tenere sincronizzati. Il costo di questa scelta è che i backup e gli upgrade di Postgres diventano responsabilità del team invece che del managed service.

**Backup: chiusi dagli Automated Backups di DigitalOcean (attivi e verificati il 02/09/2026).** Sono snapshot dell'intera droplet — quindi comprendono il volume `pgdata`, e con esso `raw_events` e `members` — presi **settimanalmente, la domenica tra le 4:00 e le 8:00 UTC**, con retention di circa 4 settimane (ultimo snapshot disponibile: `kindling-app-01 2026-08-30-04`). Il ripristino è quello nativo DO: si ricrea la droplet dallo snapshot, non si estrae il singolo database. Questo chiude il requisito "backup dal giorno 1": il `pg_dump` giornaliero verso DO Spaces previsto nella prima stesura di questo documento **non è più necessario** e non va costruito.

Il vincolo che resta è da conoscere, non da risolvere ora: con cadenza settimanale la **finestra di perdita massima è di 7 giorni** di eventi grezzi, che non sono ricostruibili a posteriori. Accettabile in questa fase (raccolta appena iniziata, nessuna decisione del community manager ancora appoggiata su questi dati).

**Alternativa considerata e rimandata (02/09/2026): cron di `pg_dump` al posto degli Automated Backups.** Un dump giornaliero del solo database, schedulato sulla droplet e spedito su object storage, darebbe una finestra di perdita di 24 ore invece di 7 giorni e permetterebbe di disattivare gli Automated Backups a pagamento. Non si fa ora, ed è una scelta consapevole, non una dimenticanza: il risparmio è di pochi euro l'anno, mentre i due backup **non coprono la stessa cosa** e lo scambio non è alla pari.

- Lo snapshot DO ripristina **l'intera macchina** (sistema, Docker, `.env`, volume dati) senza che nessuno debba aver scritto o mantenuto niente. Il `pg_dump` salva **solo il contenuto del database**: dopo un disastro va comunque ricostruita la droplet a mano prima di poterlo ripristinare.
- Il backup DO è gestito dal provider e non si rompe in silenzio; un cron è codice del team, e un cron di backup che ha smesso di funzionare è un problema che ci si accorge di avere il giorno del restore.
- Se e quando si farà, la forma giusta è **`pg_dump` in aggiunta agli snapshot DO** per un periodo di verifica (dump prodotti davvero, dimensione plausibile, restore provato almeno una volta su una droplet di prova), e solo dopo l'eventuale disattivazione degli Automated Backups. Mai il contrario: spegnere il backup che funziona prima di aver dimostrato che quello nuovo funziona è esattamente il modo in cui si resta senza nessuno dei due.

Il trigger per riaprire la questione non è il costo, è l'**RPO**: quando perdere una settimana di `raw_events` diventerà inaccettabile perché il log è la base di report su cui il community manager decide davvero.

**Hardening di rete — non opzionale, fin dal primo commit del compose file.** Su un altro progetto del team (separato da Kindling) si è verificato un incidente su MySQL in Docker: porta del database pubblicata direttamente su internet (`3306:3306`) e credenziali in chiaro in un repository GitHub pubblico — la firma tipica di attacchi automatizzati di massa contro database esposti (scanner che trovano la porta aperta, si autenticano con la password trapelata, cancellano i dati e lasciano una cartella di riscatto). La lezione riguarda la configurazione di rete e la gestione dei segreti, non la scelta del container runtime — con Docker configurato correttamente si ottiene anzi più isolamento di rete di default (un container non è raggiungibile dall'esterno a meno di pubblicarne esplicitamente la porta):
...
**Scelta rivista il 12-14/09/2026, con Streamlit scartato dopo misurazione.** Questa sezione diceva di partire con un dashboard interno in **Streamlit** per iterare in fretta con il community manager. La valutazione build-vs-riuso è stata poi fatta davvero — prevista dalla regola del progetto e dovuta prima di scrivere qualunque componente — e ha portato altrove. La decisione completa, con le misure e le alternative valutate (Grafana, Metabase, Superset, Datasette, CHAOSS 8Knot), sta in `dashboard.md`; qui restano i due motivi che riguardano l'architettura.

Il primo è l'autenticazione: il login previsto è **Discord OAuth2**, e l'autenticazione nativa di Streamlit richiede un provider OIDC con discovery document, che Discord non è. Restavano un flow scritto a mano dentro un framework che riesegue lo script a ogni interazione — un controllo di autorizzazione in un loop di re-run — oppure un auth proxy da centinaia di MB. Il secondo è la memoria: Streamlit misura **181 MB** con una sessione reale, contro i **54** di FastAPI + Jinja2 a parità di contenuto, su una macchina dove il `mem_limit` del job settimanale è 400 MB.

**La scelta è quindi FastAPI + Jinja2, server-rendered**, che ha una conseguenza architetturale non ovvia: essendo le pagine renderizzate dal server, il browser non chiama mai l'API, quindi **l'API non va mai esposta su internet** e TLS + autenticazione davanti ad essa non sono rimandati — non servono. Caddy starà davanti alla sola dashboard.

Resta valida l'evoluzione verso **Next.js** con **Sigma.js** per la visualizzazione del grafo (WebGL, regge bene migliaia di nodi, mantenuto attivamente) — Cytoscape.js resta un'opzione migliore per esplorazioni ravvicinate su sotto-grafi piccoli grazie alle interazioni più ricche — ma con un costo da vedere prima di sceglierla: un frontend JS chiama l'API **dal browser**, quindi la riespone su internet e reintroduce TLS, autenticazione e rate limiting sull'API. Anche lì vale il principio di non esposizione di profili individuali: la visualizzazione del grafo va pensata in forma anonimizzata/aggregata (es. nodi non etichettati con l'username, focus su pattern strutturali) piuttosto che come strumento per navigare la rete membro-per-membro.

Se in futuro il margine di memoria si stringe, la leva più economica prima di cambiare macchina resta generare il dashboard come pagine statiche rigenerate dal job — le metriche sono snapshot settimanali, quindi un server Python sempre vivo non è indispensabile per mostrarle. Con i 38,7 MB misurati in esercizio (sezione Hosting) quella leva non serve.

### 6. Hosting — una sola droplet, la taglia attuale

Droplet dedicata `kindling-app-01`, in un progetto DO separato "Kindling", non condivisa con altri progetti del team, per isolare risorse e falle di sicurezza tra i progetti.
...
Su quei ~630 MB entrano senza forzature i pezzi mancanti: FastAPI/uvicorn con 1 worker (~80-120 MB stimati), reverse proxy Caddy per HTTPS e autenticazione (~30 MB), la dashboard, e il job di calcolo, che non è always-on — gira qualche minuto a settimana con un picco di qualche centinaio di MB.

**Misurazione con API e dashboard in esercizio (14/09/2026), che sostituisce quelle stime:**

```
              total   used   free  buff/cache  available
Mem:          956Mi  363Mi   37Mi       584Mi      592Mi
Swap:         2.0Gi  130Mi  1.9Gi

kindling-dashboard-1   38,7 MiB  /  limite 150 MiB
kindling-api-1         41,3 MiB  /  limite 200 MiB
kindling-postgres-1    23,1 MiB
kindling-bot-1         18,6 MiB
```

Le stime erano di nuovo conservative: l'API pesa **41 MB** contro gli 80-120 previsti, la dashboard **38,7 MB** — un quarto del proprio tetto, e meno dei 54 misurati in laboratorio. I quattro container insieme fanno ~122 MB, il doppio dei ~61 MB di due settimane fa, mentre `available` è sceso solo da 634 a **592 MB**.

**Il numero che conta è il rapporto con il job**: il suo `mem_limit` è 400 MB e ne restano disponibili 592, quindi quasi 200 MB di margine sopra il tetto del processo che produce i numeri. Con Streamlit (181 MB) quel margine sarebbe stato di ~50 MB, cioè il job si sarebbe trovato a contendersi la memoria proprio mentre calcola: è questa, e non l'eleganza, la ragione della scelta di `dashboard.md` §2.

Due letture da non sbagliare su questi numeri. `free` a 37 MB non è penuria: i 584 MB di `buff/cache` sono page cache riutilizzabile, e `available` è la colonna da guardare. Lo swap passato da 69 a 130 MB non è pressione con 592 MB disponibili — è il kernel che sposta pagine fredde perché può; il segnale da sorvegliare è **swap che cresce mentre `available` cala**, non il valore assoluto.
...
- `mem_limit` per servizio nel `docker-compose.yml`: se qualcosa deve morire per esaurimento memoria deve morire il job, non il bot o Postgres. Senza limiti espliciti l'OOM killer del kernel sceglie il processo con più RSS, che è probabilmente il database.
  **Due morti diverse, da non confondere** (distinzione aggiunta il 19/09/2026, quando il bot ha ricevuto un `mem_limit: 128m` suo: fino ad allora «non il bot» era vero senza condizioni, e lo ha reso vero solo a metà quella modifica, non un errore di chi l'aveva scritto). Per l'**esaurimento di memoria della droplet** la frase sopra resta vera: nessuno spegne il bot per far posto, i tetti di job, API, dashboard e Caddy servono a far morire loro. Il bot però **può** essere fermato dal **proprio** tetto, ed è voluto: un processo di lunga durata connesso al gateway, dove una perdita di memoria crescerebbe in silenzio per giorni, diventa un riavvio visibile (`OOMKilled` in `docker inspect`, `restart: unless-stopped` lo rimette in piedi) invece di un sistema strozzato. Postgres resta senza tetto, di proposito: la sua protezione è la sua configurazione. Le ragioni e i numeri sono nel commento del `docker-compose.yml`; e il tetto del bot vale solo dopo una ricreazione del container, che il deploy non fa (`runbook-droplet.md`, procedura manuale, passo 3).
...
| Dashboard   | **FastAPI + Jinja2, server-rendered** (non Streamlit, vedi §5 e `dashboard.md`), login Discord OAuth2, dietro Caddy con TLS. In esercizio dal 14/09/2026                        | Streamlit non fa OAuth2 non-OIDC e pesa 181 MB contro 54; server-rendered significa anche che l'API non va mai esposta                       | Next.js + Sigma.js per il grafo — ma riespone l'API al browser, quindi reintroduce TLS+auth su di essa; pagine statiche dal job se serve alleggerire  |
...
- ~~**Postgres self-hosted**: backup e restore sono responsabilità del team~~ — **chiuso il 02/09/2026**: gli Automated Backups settimanali di DigitalOcean sono attivi e coprono l'intero volume dati (vedi sezione Storage). Restano due cose fuori dal backup automatico, entrambe piccole: il **restore non è mai stato provato** (uno snapshot mai ripristinato è un'ipotesi, non un backup — vale la pena verificarlo una volta, non ogni settimana), e la finestra di perdita di 7 giorni descritta nella sezione Storage.
- **Esposizione di rete e segreti**: rischio di primo livello, non secondario, alla luce dell'incidente descritto sopra — checklist di provisioning (Cloud Firewall, `.env` mai in git, nessuna porta DB pubblicata su tutte le interfacce) da rispettare fin dalla prima droplet, non da aggiungere dopo un incidente. Con API e dashboard esposti pubblicamente sulla stessa macchina la superficie cresce: TLS e autenticazione sul dashboard fanno parte della checklist, non sono un affinamento successivo.
- ~~**Margine di memoria su 1 GB**: la scelta di restare sulla taglia da 6 $/mese poggia su una misurazione fatta con i soli bot e Postgres attivi (~630 MB disponibili). Va riverificata dopo l'aggiunta di API, dashboard e job~~ — **riverificato il 14/09/2026** con API e dashboard in esercizio: `available` 592 MB, quattro container per ~122 MB totali, quasi 200 MB di margine sopra il `mem_limit` del job (vedi sezione Hosting). La taglia da 6 $/mese regge la vertical slice completa con margine, e la scelta poggia ora su due misurazioni invece che su una. **Resta aperta la parte periodica**: i segnali di resize elencati nella sezione Hosting vanno controllati ogni tanto, non una volta sola — in particolare `available` che scende stabilmente sotto ~150 MB, e lo swap che cresce mentre `available` cala. Il prossimo momento in cui riverificare è l'arrivo di Caddy (fase 2 della dashboard), che aggiunge il quinto container sempre acceso.
